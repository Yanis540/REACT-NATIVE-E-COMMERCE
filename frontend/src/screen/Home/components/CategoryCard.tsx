...
function CategoryCard({navigation,category}:CategoryCardProps) {
    ...
    return (
        <TouchableOpacity onPress={()=>navigation.navigate("Shop")}>
            <View className='flex flex-row items-center justify-start border-[1px] border-gray-200 rounded  py-1   mr-2   pr-8'>
                <View className=" mr-4 rounded">
                    <Image className="w-11 h-11 rounded-lg  object-contain  " source={{uri:category?.image??defaultSrc}} /> 
                </View>
