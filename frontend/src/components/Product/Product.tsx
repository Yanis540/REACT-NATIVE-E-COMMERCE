...
function Product({navigation,product}:ProductProps) {
    ...
    <TouchableOpacity onPress={()=>navigation.navigate("ProductDetailsScreen",{productId: product.id})} >
        <View className="flex flex-col py-4 px-2 mr-6 min-w-[250px] max-w-[270px] drop-shadow-lg border-[1px] border-gray-200 rounded-lg">
            {/* Image */}
            ...
            {/* informations : name */}
            <View className=" flex flex-row items-start py-4 justify-between">
                {/* Bg */}
                <View className="flex-[0.9] flex flex-col  ">
                    <Text style={{fontSize:17}} className="pb-4 font-bold capitalize text-lg">{product.name} </Text>
                    {/* description */}
                    <View >
